...
$skillIndexScript = Join-Path -Path $PSScriptRoot -ChildPath "skill-index.ps1"
if (Test-Path -Path $skillIndexScript) {
    Write-Verbose "Generating skill index..."
    & $skillIndexScript
} else {
    Write-Warning "Skill index script not found: $skillIndexScript"
}
