...
The original requirements from user input and not related with the project, leave it to empty if not specified
...
The environment Configration from user input (e.g. endpoint, access id), leave it to empty if not specified
...
2. **Analyze Project Structure**: Thoroughly examine the current project to understand:
    ...
    - The files need be change include depedencies, source code and configuration
    - Potential migration challenges and blockers

3. **Analysis System Enviroment**: Check the system enviroment for find the right tools for validation
...
[To Be fullfilled by scripts](NOT included this statement in task.md)