...
1. **Load context**: Retrieve the files or github issues user mentioned in the input.
    ...
    2) Collect the plan generation rules from input

2. **Generate plan**: Generate plan.md using plan-template.md, you will
...
3. **Generate tasks**: Generate task details md for each task in the plan
    1) For each task in the plan, create a task file in the corresponding task folder
    2) Find the right skill md files described in `.appmod-kit.appmod-kit/skills/skill-index.json` created by script `.appmod-kit/scripts/powershell/create-plan.ps1 -Json`, to pickup the right skill, you need to think about below rules to pickup the right skills, **DO NOT** read the skill files in the plan phase:
       - Source resource
       - Frameworks and libraries to use
       - Target resource
    3) Using the template file ".appmod-kit/templates/task-template.md", generate a task file and save the task file in the task folder with the filename task.md
    4) Run `.appmod-kit/scripts/powershell/copy-skills.ps1 -Json` with args 
    -SkillFileNames @("[skill-name-1-path]", "[skill-name-2-path]") -TaskFilePath [task-file-path] to copy reference skills from skill-index.json to the task.md

4. **Clarification**: If there are any open issues in the plan
    1) Return all the open issues to user for clarification
    2) After user clarified, update the plan and the task skills accordingly
    3) If user skip the issues, find a solution automatically

5. Make a git commit with message "chore: create modernization plan -ShortName" to save the plan.md and task skills
